...
import com.dao.TBaofeiDAO;
import com.model.TShebei;
import com.model.TBaofei;
import com.opensymphony.xwork2.ActionSupport;

public class BaofeiAction extends ActionSupport {
	private TBaofeiDAO TBaofeiDAO;
	private TShebeiDAO TShebeiDAO;
	private TBaofei baofei;
	...
	private String path;
	public TBaofeiDAO getTBaofeiDAO() {
		return TBaofeiDAO;
	}

	public void setTBaofeiDAO(TBaofeiDAO baofeiDAO) {
		TBaofeiDAO = baofeiDAO;
	}

	
	
	public TBaofei getBaofei() {
		return baofei;
	}

	public void setBaofei(TBaofei baofei) {
		this.baofei = baofei;
	}
	...
	public String baofeiUpd()
	{
		baofei.setZt(1);
		TBaofeiDAO.save(baofei);
		
		/*TShebei sb = TShebeiDAO.findById(baofei.getSbid());
		sb.setZt(0);
		TShebeiDAO.getHibernateTemplate().update(sb);*/
		
		this.setMessage("报废记录保存成功！");
		this.setPath("toBaofeiMana.action");
		return "succeed";
	}
	
	public String baofeiDetail()
	{
		List wxList = new ArrayList();
		
		String sql = "select ta.id,ta.wxsj,ta.wxnr,ta.wxr,ta.wxje,ta.bz " +
					 "from t_baofei ta where ta.sbid="+sbid;
		
		Session sqlSession = TBaofeiDAO.getSessionFactory().openSession();
		SQLQuery sqlQuery = sqlSession.createSQLQuery(sql);
		List objList = sqlQuery.list();
		for (Object object : objList) {
			Object[] obj = (Object[])object;
			
			TBaofei wx = new TBaofei();
